        ...
        CancellationToken ct)
    {
        ...
        while (cursor < endUtc)
        {
            ...
            points.Add(new ObservabilityMetricPoint
            {
                ...
                AutomationRuns = cursor <= endUtc.AddMinutes(-effectiveBucketMinutes) ? 0 : automationCounts.RanRecently,
                AutomationFailures = cursor <= endUtc.AddMinutes(-effectiveBucketMinutes) ? 0 : automationCounts.Failing,
                ProviderErrors = cursor <= endUtc.AddMinutes(-effectiveBucketMinutes) ? 0 : providerErrors,
                ProviderRetries = cursor <= endUtc.AddMinutes(-effectiveBucketMinutes) ? 0 : providerRetries,
