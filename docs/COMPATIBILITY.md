...
| Agent Plugins 1.0 portable packages | Supported with caveats | Local `plugin.json` packages are discovered from configured paths, workspace `plugins/`, and `~/.openclaw/plugins/`. OpenClaw.NET loads direct `skills/*/SKILL.md` entries and maps `mcp.json` stdio / Streamable HTTP servers into the existing MCP runtime. See [plugins/agent-plugins-1-0.md](plugins/agent-plugins-1-0.md). |
