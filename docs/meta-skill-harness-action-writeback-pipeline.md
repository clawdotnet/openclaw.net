...
- Scope: OpenClaw.NET
- Language: English
- Document: `docs/meta-skill-harness-action-writeback-pipeline.md`
...
> **External Slicer:** Temporary graphs are generated by the `openclaw graph slice` CLI command,
> built on dotNetRDF 3.5.2. It supports three data source types (SPARQL endpoints, local RDF files,
> relational databases via R2RML/Ontop) with SPARQL CONSTRUCT + JSON-LD Framing. See the
> [Graph Slicer Design Spec](superpowers/specs/2026-07-16-graph-slicer-sparql-construct-jsonld-design.md).
