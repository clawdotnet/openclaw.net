...
> 基于 TickerQ 的会话级定时循环提示词注入机制。用户设置 /loop 后，系统按指定间隔自动向会话注入预设提示词并推进 Agent 轮次，支持幂等覆盖、双层语义自毁、跨重启持久化。
...
2. 在 TickerQ 调度引擎中注册定时作业
3. 每次触发时，将预设 prompt 作为系统消息注入当前会话
4. 推进 Agent 执行完整的工具调用循环，产生响应
5. 当任务完成时（模型声明或关键词匹配），自动注销定时器

### 生态系统对比

| 特性 | Codex /loop | OpenClaw.NET /loop |
|-------|-------------|---------------------|
| 调度引擎 | 内存级定时器（随客户端消亡） | **TickerQ 持久化 cron 作业** |
| 重启恢复 | ❌ 丢失 | ✅ 自动恢复 |
| 幂等覆盖 | ✅ 单 session 唯一 | ✅ 单 session 唯一 |
| 语义自毁 | ✅ 关键词检测 | ✅ **双层检测（tool + 关键词）** |
| 触发渠道 | 仅 CLI/TUI | **CLI + 聊天频道** |
| 并发安全 | 无集群支持 | TickerQ 行锁 |
| 宿主语言 | Rust | **C# / .NET** |
...
| **Running** | 正在执行一个 loop turn | TickerQ tick 时到期条目触发 |
...
命中任一关键词即触发与路径 1 相同的终止流程。未命中则不做任何操作，loop 正常等待下一次 tick。
...
| 响应文本含关键词但任务实际未完成 | 误杀风险（保守策略）。关键词集刻意精简，避免常见词汇 |
...
| `LoopEntry.IsDue()` | 单条目内 `_nextOccurrence` 比较和更新不互锁，但在单线程 TickerQ consumer 模型中安全 |
| Agent 层 | `AgentRuntime` 内部已有会话锁，同一 session 同一时刻只有一个 turn 执行 |
| TickerQ | `[TickerFunction]` 的 cron 消费者在单线程中执行，不存在同一 tick 内竞争 |
...
| 调度引擎 | TickerQ cron 作业 | AgentRuntime 循环内联 |
...
| 持久化 | TickerQ 数据库（跨重启恢复） | InMemory + JSONL 历史 |
...
| `ClawLoopScheduler` | `ClawLoopSchedulerTests.cs` | Schedule → 条目存在；Cancel → 条目移除；SignalComplete → 通过显式接口取消；幂等覆盖；不存在的 session 返回 null；`IntervalToCron` 正确转换及非法输入抛异常 |
| `LoopTerminationDetector` | `LoopControlToolTests.cs` | 关键词命中触发 SignalComplete；不命中不触发；空/null 文本返回 false；OnToolComplete 触发 SignalComplete |
