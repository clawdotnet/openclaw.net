...
MCP App 是 OpenClaw.NET 中将第三方 MCP 应用程序作为一等公民管理的模块。它提供基于清单（manifest）的自动发现、生命周期管理、沙箱隔离和工具桥接能力，让 MCP 应用像 VS Code 扩展一样被安装、发现和使用。
...
| `transport` | string | 否 | `stdio`、`http` 或 `inprocess`，默认 `stdio` |
...
MCP App 模块包含完整的单元测试覆盖（43 个测试用例），涵盖：

| 测试类别 | 测试数 | 覆盖范围 |
|---------|--------|---------|
| 清单序列化 | 5 | JSON 往返、最小 JSON 默认值、stdio 传输、无效 JSON |
| 安装状态 | 3 | 默认值、生命周期转换时间戳、验证错误 |
| InfoProvider | 4 | 基本属性、名称回退、描述符管理 |
| 发现 | 10 | 禁用、无效路径、加载、嵌套扫描、字段验证、allow/deny、glob |
| NativeTool | 4 | HTTP 调用、带参调用、无效 JSON、数组 JSON |
| 服务器 | 7 | 连接、幂等、断开、失败、Dispose、前缀 |
